...
const ViewAll: React.FC<ViewAllProps> = ({ onBack }) => {
  ...
  const handleTabClick = (id: string, ref: React.RefObject<HTMLDivElement>, event: React.MouseEvent<HTMLDivElement>) => {
    // Lock the scroll spy
    ...
    <Box sx={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: '#F9FAFB', overflow: 'hidden' }}>
      ...
      >
        {showLeftArrow && (
          ...
          >
            <IconButton size="small" onClick={() => scrollTabBar('left')} sx={{ color: '#1C2A44', backgroundColor: '#FFFFFF', boxShadow: '1px 0 4px rgba(0,0,0,0.05)', '&:hover': { backgroundColor: '#F3F4F6' } }}>
        ...
        >
          {tabItems
            ...
            .map(tab => (
               <Box
                 key={tab.id}
                 onClick={(e) => handleTabClick(tab.id, tab.ref, e)}
                 sx={{
                   paddingBottom: '8px',
        ...
        {showRightArrow && (
          ...
          >
            <IconButton size="small" onClick={() => scrollTabBar('right')} sx={{ color: '#1C2A44', backgroundColor: '#FFFFFF', boxShadow: '-1px 0 4px rgba(0,0,0,0.05)', '&:hover': { backgroundColor: '#F3F4F6' } }}>
