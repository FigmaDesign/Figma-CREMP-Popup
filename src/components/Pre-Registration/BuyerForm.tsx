...
type BuyerRole = 'firm' | 'individual';

const buyerRoles: { value: BuyerRole; label: string; description: string }[] = [
  { value: 'firm', label: 'Firm', description: 'Purchasing on behalf of a company' },
  { value: 'individual', label: 'Individual', description: 'Personal investment' },
];
...
export default function BuyerForm({ isDesktop }: BuyerFormProps) {
  const [values, setValues] = useState<FormValues>(emptyForm);
  const [errors, setErrors] = useState<FormErrors>({});
  const [submitted, setSubmitted] = useState(false);

  const showCompanyName = values.role === 'firm';

  const validate = (): FormErrors => {
    const errs: FormErrors = {};
    if (!values.role) errs.role = 'Please select a buyer type.';
    if (!values.name.trim()) errs.name = 'Name is required.';
    if (showCompanyName && !values.companyName.trim()) {
      errs.companyName = 'Company name is required for firm registration.';
    }
  ...
  const handleRoleChange = (role: BuyerRole) => {
    setValues((prev) => ({ ...prev, role, companyName: '' }));
    setErrors((prev) => ({ ...prev, role: undefined, companyName: undefined }));
  };
